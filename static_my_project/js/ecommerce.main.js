...
form.on('submit', function(event) {
  ...
  var loadTime = 1500
  var currentTimeout;
  var errorHtml = "<i class='fas fa-warning'></i> An error occured"
  var errorClasses = "btn btn-danger disabled my-3"
  var loadingHtml = "<i class='fas fa-spin fa-spinner'></i> Loading..."
  var loadingClasses = "btn btn-success disabled my-3"
...
function stripeTokenHandler(nextUrl, token){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            card.clear()
            if (nextUrl){
                succesMsg = succesMsg + "<br/><br/><i class='fas fa-spin fa-spinner'></i> Redirecting..."
            }
            if ($.alert){
                $.alert(succesMsg)
            } else {
